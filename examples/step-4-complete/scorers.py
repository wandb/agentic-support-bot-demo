...
Return ONLY the JSON, no other text."""
    ...
    try:
        # Use Agent.from_config() helper from Slide 4.2.0
        config_path = Path(__file__).parent / "accuracy-judge-config.yaml"
        judge_agent = Agent.from_config(str(config_path))
...
Return ONLY the JSON, no other text."""
    ...
    try:
        # Use Agent.from_config() helper from Slide 4.2.0
        config_path = Path(__file__).parent / "safety-judge-config.yaml"
        judge_agent = Agent.from_config(str(config_path))
